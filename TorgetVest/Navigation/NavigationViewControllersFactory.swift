...
public class NavigationViewControllersFactory: AbstractNavigationViewControllerFactory {
    ...
    public func make(withScreenName screenName: String, arguments: PushParameters?) -> UIViewController? {
        ...
        case "events":
            ...
            viewController = UIStoryboard.articlesRootViewController(with: component, arguments: arguments)
        case "parking":
            let component = ComponentFactory.shared.make(.cars) as! CarsComponent
            viewController = UIStoryboard.carsDashboardViewController(
                with: component,
                viewModel: .init(with: arguments)
            )
        case "games":
            let component = ComponentFactory.shared.make(.games) as! GamesComponent
            viewController = UIStoryboard.gamesViewController(component: component)
        default:
            viewController = nil
        }
