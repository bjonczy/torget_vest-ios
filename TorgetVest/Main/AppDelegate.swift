...
open class AppDelegate: iOSKitPluggableApplicationDelegate, WindowProvider {
    ...
    open override func setupServices() -> [ApplicationService] {
        ...
        let games = GamesCheckServiceProvider(component: ComponentFactory.shared.make(.games) as! GamesComponent).services
        let cars = CarsApplicationServiceProvider(
            component: ComponentFactory.shared.make(.cars) as! CarsComponent
        ).services

        return [ioskit, navigation, identity, pushr, analytics, vcs, forceUpdate, cars, games].flatMap { $0 }
    }
}
